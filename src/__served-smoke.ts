 ...
 * (`createServedContext` + `runServedSession`'s `provisionAppModels` need real
 * weights; the pilot covers those). Same "prove the plumbing without a model"
 * discipline as the host tests. The reranker is no longer a Runner field — it's
 * provisioned on `RerankerCtx` by `provisionAppModels`, proven under real weights
 * by `__served-integration`.
 */
...
// ── per-session isolation: a SECOND session with its OWN config clone ──
const r2 = makeServedRunner(cfg);

 ...
 *
 * pipeline.ts can't be imported under tsx (its `.eta` prompt imports need the esbuild
...
const harnessSrc = fs.readFileSync(path.join(process.cwd(), "src/pipeline.ts"), "utf8");
const mainSrc = fs.readFileSync(path.join(process.cwd(), "src/harness.ts"), "utf8");

check("pipeline.ts: SynthPolicy renamed to DirectAnswerPolicy (shared by synth + Ask)", () => {
  assert.ok(
...
check("pipeline.ts: synth still uses the direct-answer policy", () => {
  assert.ok(
...
check("pipeline.ts: DirectAnswerPolicy keeps the free_text_return override", () => {
  assert.ok(
...
check("pipeline.ts: createResearchPolicy takes isAsk + picks the class", () => {
  assert.ok(
...
check("pipeline.ts: Ask relaxes recovery so a dropped Ask agent is salvaged, not skipped", () => {
  assert.ok(
...
check("pipeline.ts: RunResearchPlanOpts declares isAsk + policy line threads it", () => {
...
check("harness.ts: skipPlanner (Ask) branch threads isAsk to runResearchPlan", () => {
  assert.ok(
    /isAsk:\s*cmd\.skipPlanner/.test(mainSrc),
    "harness.ts skipPlanner runResearchPlan call must pass `isAsk: cmd.skipPlanner`",
  );
});

check("harness.ts: isAsk is set ONLY in the Ask branch (no Research path enables it)", () => {

#!/usr/bin/env node
/**
 * reasoning.run — the harness package.
 *
 * `harness(ctx, events, commands)` (Layer 3) is the platform entrypoint: it runs
 * the planner → research → answer pipeline over an already-established runtime
 * substrate, producing the `WorkflowEvent` stream + accepting `Command`s. It reads
 * the edge-shell concerns it can't own (live config, persistence, model-reload
 * restart, the reranker + wind-down/cancel signals) from `RunnerCtx`.
 *
 * `runMain()` (Layer 2 — the edge runner) parses CLI args, resolves config, runs
 * the model-download / `createContext` / `createReranker` boot loop and the
 * per-session restart loop, establishes the substrate, picks + binds a surface
...
import { ipc } from "@lloyal-labs/binding/node";
import {
...
// a /model, /reranker or /gpu change calls `runner.reloadRuntime` and returns,
// and the runner rebuilds on a fresh context.
...
export function* harness(
  ...
  // drawer. Closes over registry + configStore + agentEvents; re-call after
  ...
  // because the harness is what threads `appFilter` into `runQuery` /
  ...
  for (const cmd of yield* each(commands)) {
    try {
      ...
      if (cmd.type === "set_model_path") {
        ...
        // flight here. Persist + signal restart via the runner; structured
        // concurrency disposes ctx/reranker as this scope unwinds, and the
        // next iteration re-boots with the new path.
      ...
      if (cmd.type === "set_app_config") {
        ...
        // other apps, then reload so config reflects disk.
        ...
        // Every subsequent query reads config.defaults.effort.
        const saved = runner.saveConfig({
...
export function runMain(): void {
  ...
  // reconstructed KV state is exact; what's after isn't.
   ...
   *  without writing harness.json. Resolved at the boundary. */
  ...
  // injection back in as the env rung. Precedence always resolves against
  // what the user actually launched with.
  ...
  main(function* () {
    ...
    // one live research agent. One persistent signal; each run's pool subscribes fresh.
    ...
    // flag overlay. Used on every restart / config-write reload so the
    // loadConfig sites stay in one place.
    ...
    const applyGpuEnv = (cfg: Pick<LoadedConfig, "config" | "origin">): void => {
      ...
      if (gpu) {
        ...
        // leftover env value must not keep steering the native loader. This is
        // a non-fatal notice, not an error — no bus event: ui:error's reducer
        // forces uiPhase back to 'composer', which would corrupt the boot phase.
        if (!useInk && !bridgeMode) {
          process.stderr.write(
    ...
    if (useInk) {
      ...
      ipc<WorkflowEvent, Command>()(uiChannel, (cmd) => commands.send(cmd), bootstrap);
    } else {
     ...
     *  pack. Probe failures skip silently to the npm chain — the offer must
     *  never block a boot. On accept, the archives stream through the standard
     *  download UI and the SAME boot iteration proceeds. Decline persists
     *  model.backendPack=false. */
    ...
    // Each iteration spawns a child task that boots ctx + reranker, establishes
    // the substrate, sets RunnerCtx, and runs the harness. A /model, /reranker
    // or /gpu change calls `runner.reloadRuntime` (which flags `restartRequested`)
    // and the harness returns → the spawned scope tears down (reranker dispose,
    // ctx dispose, forwarder halt) before the next iteration reboots. `quit`
    // exits main(), unwinding Ink and the trace file too.
    //
    // Ink stays mounted across restarts.

    let restartRequested = false;
    let iteration = 0;
    while (true) {
      // On restart, re-load config (picks up the /model write the harness just
      // persisted via runner.reloadRuntime) and refresh Ink so the new model
      // name shows during the next load phase.
      if (iteration > 0) {
        const reloaded = reloadLiveConfig();
        liveConfig = reloaded.config;
        liveOrigin = reloaded.origin;
      ...
      const task = yield* spawn(function* (): Operation<"quit" | "restart"> {
        ...
        // Build the runner seam + hand the harness the substrate it reads.
        const runner: Runner = {
          config: () => liveConfig,
          origin: () => liveOrigin,
          saveConfig(patch) {
            const res = saveConfig(patch, configPath);
            const reloaded = reloadLiveConfig();
            liveConfig = reloaded.config;
            liveOrigin = reloaded.origin;
            return { ...res, config: liveConfig, origin: liveOrigin };
          },
          reloadRuntime(patch) {
            // Persist + null the matching CLI override so it doesn't clobber the
            // slash choice, then flag a restart. The while-loop top reloads.
            saveConfig(patch, configPath);
            if (patch.model?.path !== undefined) cliModelOverride = undefined;
            if (patch.model?.reranker !== undefined) cliRerankerOverride = undefined;
          ...
          replayCheckpoint,
          findingsMaxChars,
          mode: (!useInk && !bridgeMode) ? "oneshot" : "interactive",
          initialQuery,
          isFirstIteration: iterCaptured === 0,
        };
        yield* RunnerCtx.set(runner);
        yield* harness(ctxFinal, uiChannel, commands);
        return restartRequested ? "restart" : "quit";
      });
